...
export default function DeactivateUserModal({ open, onClose, user, onSuccess }: DeactivateUserModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(result.message?.[0] || 'Failed to deactivate user')
      }
      ...
      <DialogTitle sx={{ p: 0, mb: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <i className='ri-forbid-line' style={{ fontSize: '1.5rem', color: 'warning.main' }} />
            Desactivar Usuario
          </Box>
        ...
        <Box sx={{ mb: 2 }}>
          <Typography variant='body2' color='text.secondary' gutterBottom>
            Se desactivará al siguiente usuario:
          </Typography>
