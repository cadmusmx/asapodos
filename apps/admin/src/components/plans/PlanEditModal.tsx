...
export default function PlanEditModal({ open, onClose, plan }: PlanEditModalProps) {
  ...
  useEffect(() => {
    if (plan && open) {
      setForm({
        ...
        maxBranches: plan.limits.maxBranches != null ? String(plan.limits.maxBranches) : '',
        storageMb: plan.limits.storageMb != null ? String(plan.limits.storageMb) : '',
        supportLevel: plan.supportLevel,
        hasAdvancedReports: plan.hasAdvancedReports,
        hasBranding: plan.hasBranding
      })
      setModuleIds(Object.keys(plan.featuresById.modules).map(Number).filter(Boolean))
      setSubmoduleIds(Object.keys(plan.featuresById.submodules).map(Number).filter(Boolean))
    }
  }, [plan, open])
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Error al actualizar el plan')
    } finally {
      setLoading(false)
    }
  }
