...
const FleetStatusPieChart = ({ t, data, height = 220 }: Props) => {
    ...
    tooltip: {
      custom: ({ series, seriesIndex, w }) => {
        ...
        return `<div style="padding:8px 12px;background:var(--mui-palette-background-paper);color:var(--mui-palette-text-primary);border-radius:8px;font-size:13px;border:1px solid var(--mui-palette-divider);">
          <strong>${label}</strong><br/>${value} (${pct}%)</div>`
      }
    },
      ...
      pie: {
        ...
        donut: {
          ...
          labels: {
            ...
            value: { offsetY: -15, fontWeight: 500, fontSize: '24px', formatter: (val) => val, color: 'var(--mui-palette-text-primary)' },
            total: {
              show: true,
              fontSize: '0.875rem',
              label: t('dashboard.fleets.total'),
              color: 'var(--mui-palette-text-secondary)',
              formatter: (value) => `${(value as any)?.globals?.seriesTotals?.reduce((a: number, b: number) => a + b, 0) || 0}`
            }
          }
        }
      }
  ...
  return <AppReactApexCharts type='donut' height={height} width='100%' series={data.map(d => d.count)} options={options} />
}
