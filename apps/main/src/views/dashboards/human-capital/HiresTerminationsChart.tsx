...
type MonthlyData = { month: string; year: string; type: string; count: number }
...
const HiresTerminationsChart = ({ t, data }: Props) => {
  ...
  const altas = months.map(month => {
    const item = data.find(d => d.month === month && d.type === 'Alta')
    return item ? item.count : 0
  })

  const bajas = months.map(month => {
    const item = data.find(d => d.month === month && d.type === 'Baja')
    return item ? item.count : 0
  })
...
export default HiresTerminationsChart
