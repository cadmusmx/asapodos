...
const normalizeDecimal = (value: Prisma.Decimal | number | string | null): number => {
  if (value === null) {
    return 0
  }
...
export const normalizeInventoryItemFromRow = (row: InventoryItemRow): InventoryItemMaintenance => ({
  ...
  activeSkuCount: Number(row.ActiveSkuCount ?? 0),
...
export const normalizeInventorySkuFromRow = (row: InventorySkuRow): InventorySkuMaintenance => ({
  ...
  isActive: Boolean(row.IsActive),
  stockRecordCount: Number(row.StockRecordCount ?? 0),
  onHandQuantity: normalizeDecimal(row.OnHandQuantity),
  reservedQuantity: normalizeDecimal(row.ReservedQuantity),
  availableQuantity: normalizeDecimal(row.AvailableQuantity),
  createdAt: normalizeDateTime(row.CreatedAt),
  updatedAt: normalizeDateTime(row.UpdatedAt),
  createdBy: row.CreatedBy,
  updatedBy: row.UpdatedBy
...
export const getInventoryItemById = async (
  ...
  return rows[0] ? normalizeInventoryItemFromRow(rows[0]) : null
}

export const getInventoryItemStateById = async (
  tx: Prisma.TransactionClient,
  tenantId: string,
  itemId: number
): Promise<InventoryItemStateRow | null> => {
  const rows = await tx.$queryRaw<InventoryItemStateRow[]>(
    Prisma.sql`
                    SELECT
                        ItemID,
                        Name,
...
export const getInventorySkuById = async (
  ...
  return rows[0] ? normalizeInventorySkuFromRow(rows[0]) : null
}
