...
const WarehousesDashboard = ({ dictionary }: Props) => {
  ...
  const fetchCatalogs = async () => {
    try {
      ...
      const [regions, warehouses] = await Promise.all([regionsRes.json(), warehousesRes.json()])
    ...
    <Grid container spacing={6}>
      ...
      <Grid size={{ xs: 12, md: 6 }}>
        <KpiCard
          ...
          loading={refreshing}
        >
          <OccupancyChart t={t} counters={data.counters} occupancyPercentage={data.counters.ocupacionPorcentaje} />
        </KpiCard>
      </Grid>
      <Grid size={{ xs: 12, md: 6 }}>
        <KpiCard
      ...
      <Grid size={{ xs: 12, md: 6 }}>
        <KpiCard
          ...
          action={
            <IconButton
              size='small'
              onClick={() => setModalCapacityOpen(true)}
