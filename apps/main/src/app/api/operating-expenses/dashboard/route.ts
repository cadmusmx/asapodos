  ...
  async (req, { tenantId }) => {
    try {
      ...
      return await withTenantContext(tenantId, async () => {
        ...
        const countersResult = await prisma.$queryRaw<Array<{ total: bigint; aceptadas: bigint; pendientes: bigint; rechazadas: bigint; pagadas: bigint; montoPagadas: number }>>(
        ...
        const porMes = await prisma.$queryRaw<MonthlyData[]>(
        ...
        const porDepartamento = await prisma.$queryRaw<CountByKey[]>(
          Prisma.sql`SELECT ISNULL(d.NombreDepartamento, 'Sin asignar') as [key], COUNT(*) as [count], ISNULL(SUM(g.MontoGastado), 0) as monto FROM GASOSOL_SolGastos g LEFT JOIN GASOCO_RH_Departamento d ON g.IdDepartamento = d.IdDepartamento WHERE ${Prisma.join(baseConditionsNoEstatus, ' AND ')} GROUP BY d.NombreDepartamento ORDER BY [count] DESC`
        )
        ...
        const porPersona = await prisma.$queryRaw<CountByKey[]>(
          Prisma.sql`SELECT TOP 20 ISNULL(u.Nombre, 'Sin asignar') as [key], COUNT(*) as [count], ISNULL(SUM(g.MontoGastado), 0) as monto FROM GASOSOL_SolGastos g LEFT JOIN GASOCO_Cat_Usuarios u ON g.IdSolicitante = u.IdUsuario WHERE ${Prisma.join(porPersonaConditions, ' AND ')} GROUP BY u.Nombre ORDER BY [count] DESC`
        )
        ...
        const porTipo = await prisma.$queryRaw<CountByKey[]>(
        ...
        const porTipoPago = await prisma.$queryRaw<CountByKey[]>(
        ...
        const porProyecto = await prisma.$queryRaw<CountByKey[]>(
        ...
        const solicitadoVsGastado = await prisma.$queryRaw<MonthlyData[]>(
          Prisma.sql`
