...
function verifyBasicAuth(req: Request): boolean {
  ...
  try {
    ...
    return user === process.env.HELIX_USER && pass === process.env.HELIX_USER_PASSWORD
  } catch {
    return false
  }
}
...
export async function GET(req: Request) {
  try {
    if (!verifyBasicAuth(req)) {
      return NextResponse.json({ error: 'Credenciales incorrectas' }, { status: 403 })
    }
    ...
    if (type === 'estado') {
      ...
      const filter = IdRegion ? ` WHERE IdReg = ${IdRegion}` : ''
...
export async function POST(req: Request) {
  try {
    ...
    if (type === 'createWO') {
      const data = await queryRaw`EXEC SP_createWO_HELIX`
