...
export async function POST(req: Request) {
  try {
    ...
    const data = await queryRaw`
      ...
      INNER JOIN WorkOrders_Plantilla_Detalle_JSON DetWOR ON DetWOR.IdWO = WOT.IdWO AND DetWOR.IdPlantilla IN (${PLANTILLA_IDS[0]}, ${PLANTILLA_IDS[1]})
      ${where_enterprise}
      ${where_user}
      ${where_lfm}
      AND USU.Estatus = 'A'
