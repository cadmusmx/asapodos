...
export async function GET(req: Request) {
  try {
    ...
    const idClasificacion = searchParams.get('idClasificacion')
    ...
    const idFLM = 1
    let where = `WHERE USU.IdFLM = ${idFLM}`

    if (idUsuario) where += ` AND WOT.IdUsuario = ${idUsuario}`
    if (idClasificacion) where += ` AND WOT.IdClasificacionWO = ${idClasificacion}`
    if (idRegion) where += ` AND WOT.IdReg = ${idRegion}`
    if (idEstado) where += ` AND WOT.IdEstado = ${idEstado}`
    if (fechaInicio) where += ` AND WOT.FechaDespacho >= '${fechaInicio}'`
    if (fechaFin) where += ` AND WOT.FechaDespacho <= '${fechaFin}'`

    const data = await queryRaw`
      SELECT WOT.IdWO, WOT.Descripcion, CCW.Nombre AS ClasificacionWO, WOT.FechaDespacho, WOT.FechaLimiteConfirmacion,
             ...
             CONCAT_WS('-', USU.NombreUsuario, USU.Email) AS Usuario, WOT.IdReg, WOT.IdEstado, WOT.Avance, WOT.Alarmas,
             WOT.IcoPlantilla, WOT.IcoTarea, WOT.IcoInfo, WOT.IcoHistorial, WOT.IcoComentarios, WOT.IcoDescarga, WOT.IcoConfirmada
      FROM WorkOrders_Tickets WOT
      INNER JOIN Cat_Usuarios USU ON USU.IdUsuario = WOT.IdUsuario
...
export async function POST(req: Request) {
  try {
    ...
    if (Id_clasificacion) classificationFilter = `AND WOT.IdClasificacionWO = ${Id_clasificacion}`
    const regionFilter = Id_Region ? `AND WOT.IdReg = ${Id_Region}` : ''
    const estadoFilter = Id_Estado ? `AND WOT.IdEstado = ${Id_Estado}` : ''
    const fechaIniFilter = Fecha_inicio ? `AND WOT.FechaDespacho >= '${Fecha_inicio}'` : ''
    const fechaFinFilter = Fecha_fin ? `AND WOT.FechaDespacho <= '${Fecha_fin}'` : ''
