...
export async function GET(req: Request) {
  try {
    ...
    if (type === 'get_type_mant') {
      const data = await queryRaw`SELECT IdClasificacionWO, Nombre FROM Cat_ClasificacionWO WHERE ESTATUS = 1`
...
export async function POST(req: Request) {
  try {
    ...
    if (type === 'wo_unassigned') {
      const { Id_Usuario, Id_Region, Id_Estado, Fecha_inicio, Fecha_fin, Id_Empresa, IdPerfil, IdLFM } = body
