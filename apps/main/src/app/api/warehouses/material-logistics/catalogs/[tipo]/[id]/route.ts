...
async function resolveOwnRow(tx: Prisma.TransactionClient, tipo: CatalogKey, id: number, tenantId: string) {
  const { table, nameCol, label } = CATALOG_REGISTRY[tipo]
  const T = Prisma.raw(`dbo.${table}`)
  const N = Prisma.raw(nameCol)

  // TenantID explícito además de RLS. Se incluye el caso global (NULL) a propósito:
  // hay que VER la fila global para responder 403 con mensaje claro; filtrarla daría 404 engañoso.
...
// PUT · renombrar y/o reactivar (U). Desactivar va por DELETE (bit D).
export const PUT = withPermission<RouteCtx>('material_logistics', async (req, { auth, tenantId }, routeCtx) => {
  try {
    ...
    if (activo === false) {
      return NextResponse.json({ message: 'Para desactivar usa DELETE sobre este recurso' }, { status: 400 })
    }
    ...
    console.error('[material-logistics/catalogs PUT]', e)

    return NextResponse.json({ success: false, message: 'Ha ocurrido un error inesperado' }, { status: 500 })
  }
})

// DELETE · borrado lógico (D) -> Activo = 0. Idempotente.
export const DELETE = withPermission<RouteCtx>('material_logistics', async (_req, { auth, tenantId }, routeCtx) => {
  try {
    const { tipo, id: rawId } = await routeCtx.params
    ...
    console.error('[material-logistics/catalogs DELETE]', e)
