...
export const POST = withPermission<RouteCtx>('material_logistics', async (req, { auth, tenantId }, routeCtx) => {
  try {
    const { tipo } = await routeCtx.params
      ...
      return NextResponse.json(
        {
          ...
          ...('inactivo' in outcome && outcome.inactivo ? { inactivo: outcome.inactivo } : {})
        },
