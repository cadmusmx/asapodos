...
export const PUT = withPermission<RouteCtx>('material_validation', async (req, { auth, tenantId }, routeCtx) => {
  try {
...
export const DELETE = withPermission<RouteCtx>('material_validation', async (_req, { auth, tenantId }, routeCtx) => {
  try {
    const { tipo, id: rawId } = await routeCtx.params
