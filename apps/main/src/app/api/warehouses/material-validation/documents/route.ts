...
/**
 * Subida de archivos de Validación de Material (documentos y fotos).
 *
 * Un solo core, dos gates de permiso según el flujo:
 *   - default            → bit U (edición; comportamiento previo, retrocompatible).
 *   - ?flow=out          → bit W (alta de OutDerived).
 *
 * Form-data:
 *   - file  (requerido)  → jpg | jpeg | png | pdf, ≤ 10 MB.
 *   - name  (opcional)   → nombre semántico de FOTO (foto_material_transporte,
 *                          foto_transporte, foto_placas…). Si viene, la key va a la
 *                          carpeta por usuario como en la app; si no, se trata como
 *                          documento genérico y va a docs/web/.
 *
 * Respuesta: { success, key, url }. Se guarda la KEY en la columna; la URL es para preview.
 */
async function coreHandler(
  ...
  try {
    ...
    // Foto con nombre → carpeta por usuario (como la app):
    //   {folder}{slug}/material_validation/{userId}/{name}_{ts}{ext}
    // Documento genérico (sin name) → docs/web/ (comportamiento previo).
    const key = safeName
      ? `${folder}${slug}/material_validation/${auth.userId}/${safeName}_${ts}${ext}`
      : `${folder}${slug}/material_validation/docs/web/${ts}-web${ext}`;
...
const postWrite = withPermission('material_validation', coreHandler);
const postUpdate = withPermission('material_validation', coreHandler, { bit: PERM.U });

// ?flow=out → alta de OutDerived (W); si no, edición (U, retrocompatible).
