...
import type { InventoryListSummaryRow, InventoryStockRow } from '@/types/inventory'
  ...
  async (req, { tenantId }) => {
    ...
    try {
      return await withTenantContext(tenantId, async tx => {
        ...
        const summaryRows = await tx.$queryRaw<InventoryListSummaryRow[]>(
          Prisma.sql`
                            SELECT
        ...
        const summary = normalizeInventoryListSummary(summaryRows[0])
        ...
        return NextResponse.json({
          ...
          pagination: {
            ...
            totalPages: summary.totalRows === 0 ? 0 : Math.ceil(summary.totalRows / filters.pageSize)
          }
        })
      })
    } catch (error) {
