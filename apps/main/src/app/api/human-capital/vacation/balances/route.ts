...
export const GET = withPermission(
  ...
  async (req, { tenantId }) => {
    ...
    const result = await withTenantContext(tenantId, async tx => {
      const conditions: Prisma.Sql[] = [Prisma.sql`b.TenantID = CAST(${tenantId} AS uniqueidentifier)`]
  ...
  async (req, { auth, tenantId }) => {
    ...
    try {
      ...
      if (message.includes('UX_HC_VacationBalances_Tenant_Employee_Period')) {
        return NextResponse.json({ message: 'Ya existe un saldo activo para ese empleado y periodo.' }, { status: 409 })
      }
