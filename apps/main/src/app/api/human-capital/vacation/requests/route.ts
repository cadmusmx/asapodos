...
import type { HumanCapitalVacationRequestRow, VacationRequestStatus } from '@/types/human-capital-vacation'
...
export const GET = withPermission(
  ...
  async (req, { tenantId }) => {
    ...
    const result = await withTenantContext(tenantId, async tx => {
      const conditions: Prisma.Sql[] = [Prisma.sql`r.TenantID = CAST(${tenantId} AS uniqueidentifier)`]
  ...
  async (req, { auth, tenantId }) => {
    ...
    try {
      ...
      if (message === 'INSUFFICIENT_VACATION_BALANCE') {
        return NextResponse.json({ message: 'El empleado no tiene días disponibles suficientes.' }, { status: 409 })
      }
