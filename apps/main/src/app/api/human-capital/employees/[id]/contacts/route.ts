...
export const POST = withPermission(
  ...
  async (req, { auth, tenantId }, context: RouteContext) => {
    ...
    try {
      ...
      writeTransactionLog({
        tenantId, tableName: 'HumanCapital.EmployeeContacts', action: 'CREATE',
      ...
      if (message.includes('EMPLOYEE_NOT_FOUND')) return NextResponse.json({ message: 'El empleado no existe.' }, { status: 404 });
      if (message.includes('FK_EmployeeContacts_Relationship')) return NextResponse.json({ message: 'Parentesco inválido.' }, { status: 400 });
