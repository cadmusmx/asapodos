...
async function resolveOwnRow(tx: Prisma.TransactionClient, tipo: CatalogKey, id: number, tenantId: string) {
  const { namePk, table, nameCol, label } = CATALOG_REGISTRY[tipo]
  const T = Prisma.raw(`HumanCapital.${table}`)
  ...
  const rows = await tx.$queryRaw<CatalogRowRaw[]>`
    SELECT ${ID} AS Id, ${N} AS Nombre, TenantID, IsActive AS Activo
    FROM ${T}
...
export const PUT = withPermission<RouteCtx>('employees', async (req, { auth, tenantId }, routeCtx) => {
  try {
    ...
    console.error('[human-capital/catalogs PUT]', e)
...
export const DELETE = withPermission<RouteCtx>('employees', async (_req, { auth, tenantId }, routeCtx) => {
  try {
    const { tipo, id: rawId } = await routeCtx.params
    ...
    const outcome = await withTenantContext(tenantId, async tx => {
      ...
      await tx.$executeRaw`
          UPDATE ${T} SET IsActive = 0, UpdatedBy=${auth.userId} WHERE ${ID} = ${id} AND TenantID = ${tenantId}
        `
    ...
    console.error('[human-capital/catalogs DELETE]', e)
