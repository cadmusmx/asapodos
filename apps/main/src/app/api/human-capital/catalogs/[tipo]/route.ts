...
// GET · listar (R)
export const GET = withPermission<RouteCtx>('employees', async (_req, { tenantId }, routeCtx) => {
  try {
    ...
    const { namePk, table, nameCol } = CATALOG_REGISTRY[tipo]
    ...
    const rows = await withTenantContext(
      ...
      tx =>
        tx.$queryRaw<CatalogRow[]>`
          ...
          FROM ${T}
          ORDER BY IsActive DESC, ${N} ASC
        `
    )

    return NextResponse.json({ tipo, rows })
  } catch (e) {
    console.error('[human-capital/catalogs GET]', e)
...
export const POST = withPermission<RouteCtx>('employees', async (req, { auth, tenantId }, routeCtx) => {
  try {
    const { tipo } = await routeCtx.params
    ...
    const outcome = await withTenantContext(tenantId, async tx => {
      ...
      const inserted = await tx.$queryRaw<Array<{ Id: number }>>`
          INSERT INTO ${T} (${N}, TenantID, IsActive, CreatedBy)
          OUTPUT INSERTED.${ID}
          VALUES (${nombre}, ${tenantId}, 1, ${auth.userId})
        `
    ...
    console.error('[human-capital/catalogs POST]', e)
